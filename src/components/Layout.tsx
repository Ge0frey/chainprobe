    ...
    >
      {isActive && (
        <motion.div
          ...
          className="absolute inset-0 bg-gradient-to-r from-solana-purple/80 to-solana-teal/80 rounded-xl"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
      ...
      <div className={`relative z-10 text-xl ${isActive ? 'text-white' : ''}`}>
        {icon}
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 text-white">
      ...
      <div className="flex flex-1">
        {/* Sidebar - Desktop */}
        <aside className="hidden md:flex flex-col w-72 backdrop-blur-md bg-black/20 border-r border-white/5 overflow-y-auto">
          <div className="p-6">
            <Link to="/" className="flex items-center gap-3 mb-10">
              ...
              <div>
                <h1 className="font-bold text-xl text-white leading-none">Solana</h1>
                <p className="text-xs text-gray-400">Forensic Analysis</p>
              </div>
            </Link>
          
            <div className="space-y-1 mb-6">
              <p className="text-xs uppercase tracking-wider text-gray-500 font-medium pl-4 mb-2">Dashboard</p>
            ...
            <div className="space-y-1 mb-6">
              <p className="text-xs uppercase tracking-wider text-gray-500 font-medium pl-4 mb-2">Analysis Tools</p>
              <nav className="space-y-1 relative">
          ...
          <div className="mt-auto p-6 space-y-3">
            <button 
              onClick={openGuide} 
              className="flex items-center gap-2 px-4 py-3 w-full rounded-xl hover:bg-white/10 text-gray-300 transition-all duration-200"
            >
        ...
        {/* Mobile header */}
        <header className="md:hidden flex items-center justify-between p-4 bg-gradient-to-r from-gray-900 to-gray-800 border-b border-white/10 w-full sticky top-0 z-30">
          <Link to="/" className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-lg text-white">Solana Forensics</h1>
          </Link>
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-start justify-center pt-20"
            ...
            >
              <motion.div 
                ...
                className="w-full max-w-xl p-2 bg-gray-800/90 border border-gray-700 rounded-2xl"
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden fixed inset-y-0 right-0 z-40 w-64 bg-gray-900/95 backdrop-blur-sm border-l border-white/10 shadow-2xl"
            >
              <div className="p-5 h-full flex flex-col">
                <div className="flex justify-between items-center mb-6">
                  <p className="text-sm font-medium text-gray-400">Navigation</p>
                  <button 
                    onClick={toggleMobileMenu}
                ...
                <nav className="space-y-1 mb-6">
                  <Link to="/" 
                    className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 hover:bg-white/10 text-gray-300"
                    onClick={toggleMobileMenu}
                  >
                ...
                <div className="mt-auto flex flex-col gap-2">
                  <button 
                    ...
                    className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-white/10 text-gray-300"
                  >
                    <RiQuestionLine className="text-xl text-solana-teal" />
                    <span>Help Guide</span>
                  </button>
